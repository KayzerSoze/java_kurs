...
public class ConvertListTest {
     ...
     */
    @Test
    public void isCreateFromArraytoList() {
     ...
     */
    @Test
    public void isCreateFromListtoArray() {
        ConvertList convertList = new ConvertList();
        List<Integer> list = new ArrayList<>();
        for (int i = 1; i < 7; i++) {
            list.add(i);
    ...
    /**
     * Проверим конвертацию листа массивов int в лист Integer.
     */
    @Test
    public void isCreateListFromListInt() {
        ConvertList convertList = new ConvertList();
        List<int[]> listInt = new ArrayList<>();
        listInt.add(new int[]{1, 2});
        listInt.add(new int[]{3, 4, 5, 6});
        listInt.add(new int[]{7, 8, 9});

        List<Integer> list = new ArrayList<>();
        for (int i = 1; i < 10; i++) {
            list.add(i);
        }
        List<Integer> listTest = convertList.convert(listInt);
